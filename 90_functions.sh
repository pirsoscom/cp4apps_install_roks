...
export TEMP_PATH=/Users/nhirt/TEMP/mcm-install
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------"
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        function checkStorageClassExists() {
            ...
            then
                ...
                __output "    ${RED}  ERROR${NC}: Storage Class does not exist."
                __output "           ${ORANGE}On IBM ROKS use: ibmc-block-gold${NC}"
                __output "           ${ORANGE}On Fyre use:     rook-ceph-block-internal${NC}"
                __output "           ${ORANGE}On TEC use:      nfs-client${NC}"
   ...
   function checkRegistryCredentialsOK() {
        ...
        function printComponentsInstall() {
            ...
            if [[ $INSTALL_CP4MCM_REG_HUB == true ]]; 
            then
                __output "        '${GREEN}MCM HUB${NC}' will be registered with CP4MCM${NC}"
            else
                __output "        '${ORANGE}MCM HUB${NC}' ${RED}will NOT be registered${NC}"
...
check_and_install_helm() {
    __output "   ${magnifying} Check if ${CYAN}helm2${NC} is installed"
