...
# ---------------------------------------------------------------------------------------------------------------------------------------------------"
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        function checkHelmChartInstalled() {
            HELM_CHART=$1
            echo "    Check if ${CYAN}Helm Chart${NC} '${ORANGE}$HELM_CHART${NC}' is already installed"
    ...
    # ----^-----------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        function waitForPodsReady() {
            NAMESPACE=$1
            echo "   Waiting for Pods running in ${CYAN}Namespace${NC} ${ORANGE}$NAMESPACE${NC}. Waiting for 30 seconds...."
            sleep 30
        ...
        function waitForPodsReadyLabel() {
            NAMESPACE=$1
            LABEL=$2
            echo "   Waiting for Pods running in ${CYAN}Namespace${NC} ${ORANGE}$NAMESPACE${NC} and with ${CYAN}Label${NC} ${ORANGE}$LABEL${NC}. Waiting for 2 minutes...."
            sleep 120
